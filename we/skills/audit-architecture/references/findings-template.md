...
```
<findings_dir>/
├── <date>-hotspots.md         # Phase 1 output — written FLAT by audit-hotspots.py --write,
│                              #   NOT inside the dated directory
└── <date>-<scope>/
    ├── master.md              # entry point: Executive Summary + 3 intensity views + reading-order
    ├── cross-cutting.md       # Phase 3 output (present if any cross_cutting lens ran)
    └── subsystems/
...
4. `../<date>-hotspots.md` last (informational, candidates for next audit)
...
- [../<date>-hotspots.md](../<date>-hotspots.md) — Phase 1 architectural-density scan
...
All diagrams MUST include the severity classDef registry — the literal classDef block and the
max-severity-per-node rule live in `references/visualization.md` (the single owner).
