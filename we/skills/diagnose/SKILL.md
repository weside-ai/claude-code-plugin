...
Before exploring: check the project's own debugging rules first (e.g. a
Sentry-first reflex in `.claude/rules/`) — production evidence beats code
reading. Use the project's `CONTEXT.md` glossary if present for a clear
mental model of the relevant modules, and check ADRs in the area you're
touching.

## Phase 1 — Build a feedback loop

**This is the skill.** Everything else is mechanical. If you have a **tight** pass/fail signal for the bug — one that goes red on *this* bug — you will find the cause; bisection, hypothesis-testing, and instrumentation all just consume that signal. If you don't have one, no amount of staring at code will save you. (*Tight* per `references/design-vocabulary.md`: fast, deterministic, low-overhead, agent-runnable in one command.)
...
10. **HITL loop.** Last resort. If a human must click, drive *them* with a structured checklist script so the loop still produces captured output that feeds back to you.
...
- [ ] **Agent-runnable** — you can run it unattended; a human in the loop only via a structured checklist script.
...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling), name the change in the fix's PR and cut a ticket for it — a patch that leaves the shape untouched invites the same bug back. Make the recommendation **after** the fix is in, not before — you have more information now than when you started.
