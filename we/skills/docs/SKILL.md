...
**Counterpart:** `/we:audit-architecture` (architecture audit) · `/we:coach` (process).
...
1. If the task involved a bypass annotation change: also regenerate the
   bypass register if the script exists:
   ```bash
...
- **It does not replace `/we:audit-architecture`.** Architecture auditing
  and ADR-level drift analysis live in `/we:audit-architecture`. `/we:docs`
  handles where existing or new content goes in the documentation tree.
- **It does not maintain the bypass register by itself.** When present, the
  register is generated by `scripts/generate-bypass-register.sh`. The agent
  proposes running the generator if that script exists; skips silently
  otherwise.
