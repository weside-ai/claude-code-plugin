#!/usr/bin/env python3
"""Regression tests for checkpoint/resume robustness (PROJ-1231).
...
class CheckpointResumeTest(unittest.TestCase):
    ...
    def test_resume_next_phase_from_name(self):
        story_checkpoint("PROJ-1", "implementation_complete")
    ...
    def test_resume_picks_furthest_phase_not_latest_write(self):
        ...
        story_checkpoint("PROJ-1", "ac_verified")
    ...
    def test_list_recomputes_latest_phase_from_names(self):
        ...
        self.assertEqual(stories["PROJ-1"]["latest_phase"], "test_passed")
        self.assertFalse(stories["PROJ-1"]["is_complete"])

    def test_list_marks_complete_at_final_phase(self):
        story_checkpoint("PROJ-1", "ci_passed")
        stories = {s["story_key"]: s for s in story_list()}
        self.assertTrue(stories["PROJ-1"]["is_complete"])
