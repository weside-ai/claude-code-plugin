#!/usr/bin/env python3
"""Standalone unittest for compute_ready_set (PROJ-1231 Phase 1).
...
class ComputeReadySetTest(unittest.TestCase):
    def test_happy_path_one_ready(self):
        result = compute_ready_set([_story("PROJ-1")])
        self.assertEqual(result["ready"], ["PROJ-1"])
    ...
    def test_unrefined_no_deps_is_refinable(self):
        ...
        result = compute_ready_set([_story("PROJ-1", refined=False)])
        self.assertEqual(result["ready"], [])
        self.assertEqual(result["refinable"], ["PROJ-1"])
        self.assertEqual(result["held"], [])

    def test_already_built_held(self):
        result = compute_ready_set([_story("PROJ-1", built=True)])
        self.assertEqual(result["ready"], [])
        self.assertEqual(result["refinable"], [])
        self.assertEqual(result["held"], [{"key": "PROJ-1", "reason": "already built"}])
    ...
    def test_built_is_checked_before_refined_split(self):
        ...
        result = compute_ready_set([_story("PROJ-1", refined=False, built=True)])
        self.assertEqual(result["refinable"], [])
        self.assertEqual(result["held"], [{"key": "PROJ-1", "reason": "already built"}])

    def test_unrefined_with_refined_dep_is_refinable(self):
        # deps-refined mode: refine PROJ-2 against PROJ-1's plan/seam while PROJ-1 builds.
        # PROJ-1 is refined+not-built (ready); PROJ-2 is unrefined with dep PROJ-1 (refined) -> refinable.
        stories = [
            _story("PROJ-1", refined=True),
            _story("PROJ-2", refined=False, deps=["PROJ-1"]),
        ]
        result = compute_ready_set(stories)
        self.assertEqual(result["ready"], ["PROJ-1"])
        self.assertEqual(result["refinable"], ["PROJ-2"])
        self.assertEqual(result["held"], [])

    def test_unrefined_with_built_dep_is_refinable(self):
        # A built dep also satisfies the refine-dependency.
        stories = [
            _story("PROJ-1", built=True),
            _story("PROJ-2", refined=False, deps=["PROJ-1"]),
        ]
        result = compute_ready_set(stories)
        self.assertEqual(result["refinable"], ["PROJ-2"])
    ...
    def test_unrefined_with_unmet_dep_is_held_not_refinable(self):
        # PROJ-2 is unrefined and its dep PROJ-3 is neither refined nor built -> held,
        # NOT refinable (don't refine ahead of an unmet dep).
        stories = [
            _story("PROJ-2", refined=False, deps=["PROJ-3"]),
            _story("PROJ-3", refined=False),
        ]
        result = compute_ready_set(stories)
        self.assertEqual(result["ready"], [])
        # PROJ-3 has no deps -> refinable; PROJ-2 waits on PROJ-3.
        self.assertEqual(result["refinable"], ["PROJ-3"])
        self.assertEqual(result["held"], [{"key": "PROJ-2", "reason": "waiting on PROJ-3"}])

    def test_unmet_dependency_held(self):
        result = compute_ready_set([_story("PROJ-2", deps=["PROJ-1"])])
        self.assertEqual(result["ready"], [])
        self.assertEqual(result["held"], [{"key": "PROJ-2", "reason": "waiting on PROJ-1"}])

    def test_cap_hit(self):
        stories = [_story("PROJ-1"), _story("PROJ-2"), _story("PROJ-3")]
        result = compute_ready_set(stories, cap=2)
        self.assertEqual(result["ready"], ["PROJ-1", "PROJ-2"])
        self.assertEqual(result["held"], [{"key": "PROJ-3", "reason": "cap reached"}])

    def test_built_dependency_satisfies_dep(self):
        stories = [
            _story("PROJ-1", built=True),
            _story("PROJ-2", deps=["PROJ-1"]),
        ]
        result = compute_ready_set(stories)
        self.assertEqual(result["ready"], ["PROJ-2"])
        self.assertEqual(result["held"], [{"key": "PROJ-1", "reason": "already built"}])
...
class ResolveEpicIdentifiersTest(unittest.TestCase):
    ...
    def test_resolves_by_key_against_slug_named_file(self):
        with tempfile.TemporaryDirectory() as d:
            self._make_epic_plan(d, "docs-portal", "PROJ-1263")
            ids = _resolve_epic_identifiers("PROJ-1263", Path(d))
            # The key must pull in the slug so slug-keyed stories are matched.
            self.assertIn("docs-portal", ids)
            self.assertIn("PROJ-1263", ids)
    ...
    def test_unrelated_epic_not_pulled_in(self):
        with tempfile.TemporaryDirectory() as d:
            self._make_epic_plan(d, "docs-portal", "PROJ-1263")
            self._make_epic_plan(d, "billing", "PROJ-2000")
            ids = _resolve_epic_identifiers("PROJ-1263", Path(d))
            self.assertNotIn("billing", ids)
            self.assertNotIn("PROJ-2000", ids)
