...
Consumers: `/we:develop`, `/we:orchestrate`, the `test-runner` agent, worker briefs (which inline
the level so a detached worker needs no reference), and the bug-hunt engine at integration (Codex
