...
| 2 · User sees widgets and can tap **Create widget** | Met | `apps/mobile/src/screens/WidgetsScreen.tsx` renders the list and the button, calls the endpoint; screen registered in the navigator (same diff); plan `## Verification` walkthrough covers the tap |
