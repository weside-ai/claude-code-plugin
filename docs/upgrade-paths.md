...
**What you have:** the plugin alone. All 22 skills. The full `/we:story → /we:build → merge` pipeline (with `/we:vision`, `/we:saga`, `/we:epic` upstream when you start higher). Councils with nine generic role-lenses. Meetings at four altitudes.
